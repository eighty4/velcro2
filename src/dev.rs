...
impl VelcroDevApi {
    ...
    fn dispatch(&self, cmd: VelcroDevApiCommand) {
        match cmd {
            VelcroDevApiCommand::DeleteMapping(index) => self.runtime.block_on(self.delete_mapping(index))
        }
    }

    async fn create_mapping(&self, index_name: &str) -> () {
        self.es_client.indices().create(IndicesCreateParts::Index(index_name)).send().await.expect("create index");
    }

    async fn delete_mapping(&self, index_name: &str) -> () {
        self.es_client.indices().delete(IndicesDeleteParts::Index(&[index_name])).send().await.expect("delete index");
    }
}
